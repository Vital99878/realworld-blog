    ...
    <form className={cn(CLASS_NAME)} onSubmit={handleSubmit(onSubmit)}>
      ...
      <label className={cn(`${CLASS_NAME}__label`)}>
        ...
        <input
          ...
          ref={register({
            validate: (value) => value === password.current || <Error text="The passwords do not match" />,
            required: true,
            minLength: 8,
            maxLength: 40,
          })}
        ...
        {errors.repeatPassword?.type === 'minLength' && (
          <Error text="Your password needs to be at least 6 characters" />
        )}
      </label>
      <label className={cn(`${CLASS_NAME}__label-checkbox`)}>
        ...
        />
        <div>
          <span className={cn(`${CLASS_NAME}__input-checkbox-text`)}>
